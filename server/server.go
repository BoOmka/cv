...
import (
	...
	"github.com/boomka/cv/app/pages"
...
func runDevServer(addr string) {
	...
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		werr.WriteError(w, func() error {
			...
			f, err := http.Dir(filepath.Join(dir, "dist")).Open(p)
			if os.IsNotExist(err) {
				http.NotFound(w, r)
				return nil
			} else if err != nil {
...
func staticGenerate(outDir string) error {
	...
	for _, p := range pathList {
		...
		var rbuf bytes.Buffer
		renderer := staticrender.New(&rbuf)
		application, rootBuilder := app.VuguSetup(buildEnv, renderer.EventEnv(), nil)

		req, err := http.NewRequest("GET", "http://localhost"+p, nil)
		if err != nil {
			return err
		}
		application.Router.ProcessRequest(req)
		...
		err = os.WriteFile(fileName, rbuf.Bytes(), 0644)
		if err != nil {
			return err
		}
